...
def data_generator(batch_size):
	...
	FOLDERPATH = DATAPATH + FOLDER
	FOLDERPATH = FOLDERPATH + 'images/'
	images_dir = glob.glob(FOLDERPATH + '*.jpg')
	random.shuffle(images_dir)
	...
	for i in range(batch_size):
		...
		img_seg_pad_w, _, _ = dlk.warp_hmg(img_seg_pad.unsqueeze(0), dlk.H_to_param(inv_func.forward(dlk.param_to_H(p_gt))))
...
def test():
	...
	for i in range(10):
		img_to_write = img_batch[i, :, :, :]
		cv2.imwrite('test_result/img_' + str(i) + '.jpg',
					255 * np.array(torch.squeeze(img_to_write)).transpose([1, 2, 0]))
		tmpl_to_write = template_batch[i, :, :, :]
		cv2.imwrite('test_result/tmpl_' + str(i) + '.jpg',
					255 * np.array(torch.squeeze(tmpl_to_write)).transpose([1, 2, 0]))
...
def train():
	...
	parameters = dlk_net.conv_func.parameters()
	optimizer = optim.Adam(filter(lambda p: p.requires_grad, parameters), lr=0.0001)
...
def self_train():
	...
	for i in range(num_minibatch):
		...
		img_batch_unnorm, template_batch_unnorm, training_param_batch = data_generator(minibatch_sz)
		valid_img_batch, valid_template_batch, valid_param_batch = data_generator(minibatch_sz)
		start = time.time()
		optimizer.zero_grad()
